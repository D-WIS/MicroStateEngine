    ...
    public class Program
    {
        ...
        static void Main()
        {
            ...
            if (assembly != null)
            {
                ...
                if (dir != null)
                {
                    string tempFile = Path.Combine(dir.FullName, "SemanticQueriesZeroAxialVelocityTopOfStringThreshold.md");
                    using (StreamWriter writer = new StreamWriter(tempFile))
                    {
                        var queries1 = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(Thresholds).FullName, "ZeroAxialVelocityTopOfStringThreshold");
                        GenerateSparQLForMD(writer, "ZeroAxialVelocityTopOfStringThreshold", queries1);
                    }
                }
            }
        }
